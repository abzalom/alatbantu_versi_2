...
class ApiAuthToken
{
    ...
    {
        if (!$request->has('extension_api') || !$request->extension_api) {
            ...
            // lakukan login dengan username admin dan password untuk authentication extension api
            if (!Auth::attempt(['username' => $request->username_api, 'password' => $request->password_api])) {
                return response()->json(['User tidak ditemukan'], 401);
            }
            return $next($request);
        }
