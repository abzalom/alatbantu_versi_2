$(document).ready(function () {
    const formName = ["sumberdana", "vol_subkeg", "anggaran", "mulai", "selesai", "penerima_manfaat", "jenis_layanan", "jenis_kegiatan", "ppsb", "multiyears", "dana_lain", "lokus", "koordinat", "keterangan", "catatan", "opd_tag_otsus", "subkegiatan", "file_kak_name", "file_rab_name", "file_pendukung1_name", "file_pendukung2_name", "file_pendukung3_name", "link_file_dukung_lain"];

    let jenisKegiatan = ""; // Variabel untuk menyimpan jenis kegiatan yang dipilih

    const formToDisabled = ["#new-opd_tag_otsus", "#new-subkegiatan", "#new-vol_subkeg", "#new-anggaran", "#new-penerima_manfaat", "#new-jenis_layanan", "#new-jenis_kegiatan", "#new-ppsb", "#new-multiyears", "#new-mulai", "#new-selesai", "#new-dana_lain", "#new-lokus", "#new-koordinat", "#new-keterangan", "#new-file-kak", "#new-file-rab", "#new-file-pendukung1", "#new-file-pendukung2", "#new-file-pendukung3", "#new-link-file-dukung-lain"];
    ...
    $("#detailRapOpdModal").on("hidden.bs.modal", function () {
    ...
    $(".btn-detail-rap").on("click", function () {
        if (auth.username.toLowerCase() === "admin") {
            // Tambahkan logika khusus untuk admin di sini
            if (!opdTimPembahas.length && tahunAnggaran > 2025) {
                setTimeout(() => {
                    $("#detailRapOpdModal").modal("hide");
                    showToast("Belum ada tim pembahas. Silakan tambahkan tim pembahas terlebih dahulu.", "danger");
                }, 1000);
                return;
            }
        }
    ...
    /**
     * Script Untuk Modal Tim Pembahas RAP OPD
     */

    var timPembahasModal = ["modalTimPembahasRapOpd", "modalListTimPembahas", "modalNewTimPembahas", "modalNewTimPembahasOpd"];

    timPembahasModal.forEach(function (modalId) {
        $("#" + modalId).on("hidden.bs.modal", function () {
            $("#modal-" + modalId + "-show-spinner").show();
            $("#modal-" + modalId + "-show-content").hide();
            $(".chkTimPembahas").prop("checked", false);
            if (modalId === "modalNewTimPembahas") {
                $("#formNewTimPembahas").attr("action", `/config/team/bappeda`);
                $("#httpMethodNewTimPembahas").val("POST");
                // reset form with id formNewTimPembahas
                $("#formNewTimPembahas")[0].reset();
            }
            if (modalId === "modalNewTimPembahasOpd") {
                $("#formNewTimPembahasOpd").attr("action", `/config/team/opd`);
                $("#httpMethodNewTimPembahasOpd").val("POST");
                // reset form with id formNewTimPembahasOpd
                $("#formNewTimPembahasOpd")[0].reset();
            }
        });

        $("#" + modalId).on("shown.bs.modal", function () {
            setTimeout(() => {
                $("#modal-" + modalId + "-show-spinner").hide();
                $("#modal-" + modalId + "-show-content").show();
            }, 1000);
        });
    });

    $("#formListTimPembahas").on("submit", function (e) {
        const selectedTim = $(".chkTimPembahas:checked")
            .map((_, el) => $(el).val())
            .get();

        if (selectedTim.length === 0) {
            e.preventDefault(); // cegah submit hanya saat invalid
            showToast("Pilih minimal satu anggota tim pembahas.", "danger");
            return;
        }

        let countTimM = opdTimPembahas.length + selectedTim.length;

        if (countTimM > 5) {
            e.preventDefault(); // cegah submit hanya saat invalid
            showToast(`Maksimal 5 anggota. tim yang anda sudah mencapai ${countTimM} anggota.`, "danger");
            return;
        }
    });

    $(".remove-tim-bappeda").on("click", function () {
        const timId = $(this).data("tim_id");
        const opdId = $(this).data("opd_id");
        $(this).closest(".tim-bappeda-item").remove();
        $(".tim-bappeda-item").each(function (index) {
            let timId = $(this).data("tim_id");
            $(this)
                .find(`.urutan-tim-${timId}`)
                .text(index + 1);
        });
        // Hapus dari array opdTimPembahas
        opdTimPembahas = opdTimPembahas.filter((tim) => tim.id !== timId);
        console.log(opdTimPembahas);
        // kirim ajax untuk hapus di database
        $.ajax({
            type: "POST",
            url: appApiUrl + "/api/config/tim_pembahas/bappeda/destroy_member",
            data: {
            ...
            success: function (response) {
                ...
                if (response.success) {
                    showToast(response.message, response.alert || "success");
                }
            },
            error: function (xhr) {
                let errorResponse = handleAjaxError(xhr);
    ...
    $("#searchListTim").on("input", function () {
        const searchTerm = $(this).val().toLowerCase();
        // cari pada  <tbody id="listTimPembahas">
        $("#listTimPembahas tr").filter(function () {
            $(this).toggle($(this).text().toLowerCase().indexOf(searchTerm) > -1);
        });
    });

    $(".btn-edit-tim-pembahas").on("click", function () {
        const member = $(this).data("member");
        console.log(member);
        // set action untuk form dengan id formNewTimPembahas ke /config/team/bappeda/{team} dengan method PUT
        $("#formNewTimPembahas").attr("action", `/config/team/bappeda/${member.id}`);
        $("#httpMethodNewTimPembahas").val("PUT");
        $("#inputNamaTimPembahas").val(member.nama);
        $("#inputNipTimPembahas").val(member.nip);
        $("#inputJabatanTimPembahas").val(member.jabatan);
        // Lakukan sesuatu dengan member, misalnya ambil data anggota tim
    });

    $("#formNewTimPembahas").on("submit", function (e) {
        e.preventDefault();
        const nama = $("#inputNamaTimPembahas").val();
        const nip = $("#inputNipTimPembahas").val();
        const jabatan = $("#inputJabatanTimPembahas").val();
        // validasi input tambahkan class is-invalid pada input jika kosong
        if (!nama) {
            $("#inputNamaTimPembahas").addClass("is-invalid");
            $("#inputNamaTimPembahas_error").html("Nama Tim Pembahas wajib diisi");
            return;
        } else {
            $("#inputNamaTimPembahas").removeClass("is-invalid");
            $("#inputNamaTimPembahas_error").html("");
        }
        // cek jika nip ada isinya, maka harus validasi nip (angka dan 18 karakter)
        if (nip) {
            const nipRegex = /^\d{18}$/;
            if (!nipRegex.test(nip)) {
                $("#inputNipTimPembahas").addClass("is-invalid");
                $("#inputNipTimPembahas_error").html("NIP harus berupa 18 digit angka");
                return;
            } else {
                $("#inputNipTimPembahas").removeClass("is-invalid");
                $("#inputNipTimPembahas_error").html("");
            }
        } else {
            $("#inputNipTimPembahas").removeClass("is-invalid");
            $("#inputNipTimPembahas_error").html("");
    ...
    $(".edit-tim-opd").on("click", function () {
        const member = $(this).data("member");
        console.log(member);
        if (member) {
            $("#formNewTimPembahasOpd").attr("action", `/config/team/opd/${member.id}`);
            $("#httpMethodNewTimPembahasOpd").val("PUT");
            $("#inputNamaTimPembahasOpd").val(member.nama);
            $("#inputNipTimPembahasOpd").val(member.nip);
            $("#inputJabatanTimPembahasOpd").val(member.jabatan);
        }
    });
});
